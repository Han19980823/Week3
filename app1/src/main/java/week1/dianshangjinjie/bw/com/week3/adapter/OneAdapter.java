 ...
 */
public  class OneAdapter extends RecyclerView.Adapter<OneAdapter.fisAdapter> {
    Context context;
    List<SQentity.ResultBean> list;

    public OneAdapter(Context context,   List<SQentity.ResultBean> list) {
    ...
    public fisAdapter onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view1 = LayoutInflater.from(context).inflate(R.layout.one_item_layout, parent, false);
        return new fisAdapter(view1);
    }

    @Override
    public void onBindViewHolder(@NonNull fisAdapter holder, int position) {
        Glide.with(context).load(list.get(position).getHeadPic()).into(holder.img_view1);
        holder.text_name1.setText(list.get(position).getSignature());
        holder.text_name2.setText(list.get(position).getUserId()+"");
        holder.text_name3.setText(list.get(position).getNickName()+"");
